...
import { getArticleCategoryList, deleteArticleCategory } from "@/services/ant-design-pro/articleCategories";
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space split={<Divider type="vertical" />}>
          ...
          <Save title="新建分类" initialValues={{ parentId: record.articleCategoryId }} onFinish={() => {
            ref.current?.reload();
          }}>
            <a>
              新建子分类
          ...
          <Popconfirm title="您确定删除吗？" onConfirm={async () => {
            const res = await deleteArticleCategory({ articleCategoryId: record.articleCategoryId })
            if (res.code === 1) {
              ref.current?.reload();
              return
            }
